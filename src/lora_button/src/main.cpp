...
#define BUTTON_VALUE_ON    0
#define BUTTON_VALUE_OFF   1
...
int main() {
    ...
    while (true) {
        ...
        if (filterGpioValue(triggerGpio.read()) == BUTTON_VALUE_ON) {
            logInfo("Sending command");
            send_data(CMD_BUTTON, (uint8_t)sizeof(CMD_BUTTON));
        }
